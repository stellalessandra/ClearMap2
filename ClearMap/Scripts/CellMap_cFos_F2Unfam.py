...
if __name__ == "__main__":
  ...
  directory = '/data01/astella/projects/SexualImprinting/C57_MaleUrine_Exposure_cFos/F2Unfam/'   
...
#      "fixed_image"  : ws.filename('resampled'),
  ...
  slicing = (slice(3160,3360),slice(4270,4470),slice(670,700));
...
#  io.mhd.write_header_from_source(ws.filename('stitched', postfix='illumination'))
  ...
  cells.filter_cells(source = ws.filename('cells', postfix='700', prefix='debug'), 
                     sink = ws.filename('cells', postfix='filtered_700', prefix='debug'), 
...
#  #%%
  ...
  io.write(ws.filename('cells',postfix='overlap', prefix='debug'), np.asarray(aa, order='F'))
  io.mhd.write_header_from_source(ws.filename('cells', postfix='overlap', prefix='debug'))
...
#  io.mhd.write_header_from_source(ws.filename('stitched', postfix='illumination'))
  ...
  source = ws.source('cells', postfix='filtered_700')
...
#  To compare data from different animals we need to move coordinates back to the same reference atlas
  ...
  #%%
#  io.convert_files('density_intensities.tif', 'npy')
